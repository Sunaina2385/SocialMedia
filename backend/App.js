...
app.post('/logout', async (req, res) => {
    ...
    try {
        // Here, instead of verifying the token, you can just blacklist it
        // In a real application, you'd likely store revoked tokens in a database or cache
        // For simplicity, I'm using an array here
        const tokenValue = token.split(' ')[1]; // Extract token without 'Bearer'
        jwt.verify(tokenValue, 'YOUR_SECRET_KEY', (err, decoded) => {
            if (err) {
                return res.status(401).send('Invalid token');
            }
            
            // Check if token is already revoked
            if (revokedTokens.includes(tokenValue)) {
                return res.status(401).send('Token already revoked');
            }
            
        revokedTokens.push(tokenValue);
